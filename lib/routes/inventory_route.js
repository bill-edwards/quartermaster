...
router.route('/inventory/:invId')
	...
	.get(gatekeeper, function(req, res, next){
		...
		if (!(/^\d+$/.test(req.params.invId))) return next (new AppError(404, 'id must be a number'));
		...
		Inventory.belongsToUser(req.params.invId, req.session.userId, function(err, belongs){
			...
			Inventory.findById(req.params.invId, function(err, inventory){
				if (err) return next(err);
				// Attach details of inventory's items. 
	...
	.put(gatekeeper, bodyParser.json(), validate('inventory',[]), function(req, res, next){
...
router.route('/inventory/new')
	...
	.post(gatekeeper, bodyParser.json(), validate('inventory',['name']), function(req, res, next){
