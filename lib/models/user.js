...
User.findEvents = function(id, callback){
	...
	dbConn.query("SELECT events.* FROM events JOIN users_events ON users_events.eventId = events.id WHERE users_events.userId='"+id+"' ORDER BY events.startDate", function(err, results){
		if (err) return callback(err);
		return callback(null, results);
	});
};
