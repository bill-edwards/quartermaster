...
User.findInventories = function(id, callback){
	...
	dbConn.query("SELECT inventories.* FROM inventories JOIN users_inventories ON users_inventories.invId = inventories.id WHERE users_inventories.userId='"+id+"' ORDER BY inventories.name", function(err, results){
		if (err) return callback(err);
		return callback(null, results);
	});
};
