...
function create(callback){
	//if (this.hasOwnProperty('id')) return callback(null); // or should throw error? 
	var inv = this; 
	dbConn.query('INSERT INTO inventories (name) VALUES ("'+this.name+'")', function(err, result){
		if (err) return callback(err); 
		inv.id = result.insertId; 

		dbConn.query('INSERT INTO users_inventories (userId, invId) VALUES ("'+inv.user+'", "'+inv.id+'")', function(err, result){
			if (err) return callback(err);
			callback(null);
		});
	});
