...
Inventory.belongToUserFilter = function(invIds, userId, callback){
	...
	// Construct array of ids of inventories belonging to user. 
	dbConn.query("SELECT invId FROM users_inventories WHERE users_inventories.userId ='"+userId+"'", function(err, invs){
		if (err) return callback(err);
